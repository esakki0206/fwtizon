...
import { FiPlus, FiVideo, FiUsers, FiMessageCircle, FiTrash2, FiUploadCloud, FiDownload } from 'react-icons/fi';
...
const LiveCourseManager = () => {
  ...
  const [currentCourse, setCurrentCourse] = useState(null); // track full course object for applications view
  ...
  const handleExportApplications = async () => {
    ...
    try {
      const res = await axios.get(
        `/api/admin/live-courses/${editingCourse}/applications/export`,
        { responseType: 'blob' }
      );
  ...
  // ── Resolve display instructor name for a row ──────────────────────────────
  const resolveInstructorName = (row) =>
    (row.instructorName && row.instructorName.trim()) ||
    row.instructor?.name ||
    null;
  ...
  const columns = [
      ...
      cell: (row) => {
        ...
        return (
          <div className="min-w-0 max-w-xs">
            <div className="font-bold text-gray-900 dark:text-white truncate">{row.title}</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-1 truncate">
              <FiUsers size={10} className="shrink-0 opacity-60" />
              <span className="truncate">
                {instructorName ? (
                  <span className="font-medium text-primary-600 dark:text-primary-400">{instructorName}</span>
                ) : (
                  <span className="italic opacity-60">Instructor N/A</span>
                )}
              </span>
            </div>
          </div>
      ...
      cell: (row) => (
        <div className="min-w-0">
          <div className="text-gray-700 dark:text-gray-200 font-medium text-sm">
            {formatLiveCourseStartLabel(row, { month: 'short', day: 'numeric', year: 'numeric', includeTime: false })}
          </div>
          {getLiveCourseTimingText(row) ? (
            <div className="text-[11px] text-gray-500 mt-0.5">{getLiveCourseTimingText(row)}</div>
          ) : null}
        </div>
      ),
    },
    {
      ...
      cell: (row) => (
        <span className="bg-primary-50 dark:bg-primary-900/40 text-primary-600 dark:text-primary-400 px-2 py-0.5 rounded font-bold text-xs">
          {row.currentEnrollments || 0} / {row.maxStudents || 30}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={(e) => handleViewApplications(e, row)}>
            Applications
          </Button>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="mt-1 w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      className="mt-1 w-full h-24 resize-none bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative mt-1">
                      ...
                      <input
                        ...
                        className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-primary-500"
                      />
                    </div>
                  </div>
                  <div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="mt-1 w-full h-20 resize-none bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                    ...
                    <div className="flex items-center space-x-4">
                      {formData.instructorImage ? (
                        <img
                          ...
                          className="w-16 h-16 rounded-full object-cover shadow-sm border border-gray-200 dark:border-gray-700"
                        />
                      ) : (
                      ...
                      <div>
                        ...
                        <label
                          ...
                          className="cursor-pointer inline-flex items-center px-3 py-1.5 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition"
                        >
                  ...
                  <CardContent>
                    <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700">
                      <div className="flex items-center">
                        ...
                        <span className="font-bold text-gray-900 dark:text-white">Enrolled</span>
                      </div>
                      <span className="font-medium text-sm text-gray-500">
                        {liveCourses.find(c => c._id === editingCourse)?.currentEnrollments || 0} / {formData.maxStudents}
                      </span>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="text-xs text-gray-500 uppercase bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        ...
                        <th className="px-4 sm:px-6 py-4 font-bold">Applicant</th>
                        <th className="px-4 sm:px-6 py-4 font-bold">Contact</th>
                        <th className="px-4 sm:px-6 py-4 font-bold hidden md:table-cell">Gender</th>
                        <th className="px-4 sm:px-6 py-4 font-bold hidden lg:table-cell">Background</th>
                        <th className="px-4 sm:px-6 py-4 font-bold">Payment</th>
                        <th className="px-4 sm:px-6 py-4 font-bold">Applied On</th>
                      </tr>
                    </thead>
                      ...
                      {applications.map((app, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 sm:px-6 py-4">
                            ...
                            <div className="text-gray-500 text-xs mt-0.5">WA: {app.whatsappNumber}</div>
                          </td>
